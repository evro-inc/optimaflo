...
************************************************************************************/
export async function listGACustomMetrics(skipCache = false): Promise<any[]> {
  ...
  try {
    ...
    const properties = cleanedData.flatMap((item) => item.customMetrics || []); // Flatten to get all properties directly
...
export async function deleteGACustomMetrics(
  ...
  if (notFoundLimit.length > 0) {
    return {
      ...
      )}. All other metrics were successfully deleted.`,
  ...
  return {
    success: true,
    message: `Successfully deleted ${successfulDeletions.length} metric(s)`,
...
************************************************************************************/
export async function createGACustomMetrics(formData: {
  forms: CustomMetricSchemaType['forms'];
}): Promise<FeatureResponse> {
  const userId = await authenticateUser();
  const token = await getOauthToken(userId);
  const { tierLimitResponse, availableUsage } = await checkFeatureLimit(
  ...
  if (tierLimitResponse.limitReached || formData.forms.length > availableUsage) {
    return {
      ...
      errors: [
        `Cannot create more metrics than available. You have ${availableUsage} creations left.`,
      ],
      results: [],
    };
  }
  ...
  await Promise.all(
    formData.forms.map(async (data) => {
      ...
      const url = `https://analyticsadmin.googleapis.com/v1beta/${cleanedData.property}/customMetrics`;
      ...
      try {
        const res = await executeApiRequest(url, {
          ...
          body: JSON.stringify({
            ...
            measurementUnit: cleanedData.measurementUnit,
            restrictedMetricType: cleanedData.restrictedMetricType,
          }),
        });
  ...
  return {
    success: true,
    message: `Successfully created ${successful.length} custom dimension(s)`,
...
************************************************************************************/
export async function updateGACustomMetrics(formData: {
  forms: CustomMetricSchemaType['forms'];
}): Promise<FeatureResponse> {
  const userId = await authenticateUser();
  const token = await getOauthToken(userId);
  ...
  if (tierLimitResponse.limitReached || formData.forms.length > availableUsage) {
    return {
      ...
      errors: [
        `Cannot update more metrics than available. You have ${availableUsage} updates left.`,
      ],
      results: [],
    };
  }
  ...
  await Promise.all(
    formData.forms.map(async (data) => {
      ...
      const updateFields: string[] = ['description', 'displayName', 'restrictedMetricType'];

      if (cleanedData.measurementUnit !== 'CURRENCY') {
      ...
      if (cleanedData.measurementUnit !== 'CURRENCY') {
        requestBody.measurementUnit = cleanedData.measurementUnit;
      }

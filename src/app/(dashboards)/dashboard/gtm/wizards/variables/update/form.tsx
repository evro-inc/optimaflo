...
const FormUpdateVariables: React.FC<FormUpdateProps> = React.memo(
  ({ tierLimits, accounts, containers, workspaces }) => {
    ...
    useErrorRedirect(selectedRowDataFiltered, router, '/dashboard/gtm/configurations');

    const formDataDefaults: Variable[] = Object.values(selectedRowDataFiltered).map((rowData) => ({
                    ...
                    {selectedType &&
                      (() => {
                        switch (selectedType) {
                          ...
                          case 'ctv':
                            return (
                              <div>No configuration required for container version number</div>
                            );
                          default:

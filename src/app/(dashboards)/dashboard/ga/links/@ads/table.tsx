...
import { GoogleAdsLink } from '@/src/types/types';
...
export function DataTable<TData, TValue>({ columns, data }: DataTableProps<TData, TValue>) {
  ...
  const getDisplayNames = (items) => items.map((item: GoogleAdsLink) => item.name);
  const handleDelete = useDeleteHook(
    deleteGAGoogleAdsLinks,
    selectedRowData,
    table,
    getDisplayNames,
    'ad link'
  ...
  const refreshAllCache = () => {
    startRefreshTransition(async () => {
      try {
        ...
        const keys = [`ga:ads:userId:${userId}`];
  ...
  return (
    <div className="container mx-auto p-4">
      <h2 className="text-2xl font-bold mb-4">Google Ad Links</h2>
      <div className="flex flex-col sm:flex-row justify-between items-center mb-4 space-y-2 sm:space-y-0 sm:space-x-2">
        <Input
          placeholder="Filter ad link names..."
